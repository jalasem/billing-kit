...
// `route.ts` builds its providers via `createProvider`, which needs real
// Stripe/Paystack env vars and, once built, would make a live `listSettlements`
// call. Mocked here so the "200 on the correct secret" path never touches
// either provider — only the auth check and the reconcile job itself
// (against Postgres, with a fake provider) are under test.
...
describe("POST /api/cron/reconcile", () => {
  ...
  it("returns 200 with summaries for the correct secret, never touching a live provider", async () => {
    process.env.CRON_SECRET = "correct-secret";
